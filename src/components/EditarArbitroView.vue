...
const props = defineProps(['arbitro']);
const emit = defineEmits(['fecharEditarArbitro', 'atualizar']);
const login = ref('');
const nome = ref('');
const senha = ref('');
...
watch(
  ...
  (nova) => {
    if (nova) {
      login.value = nova.login_arbitro || '';
      nome.value = nova.nome_arbitro || '';
      senha.value = nova.senha_arbitro || '';
    }
  },
  { immediate: true }
);
function checarDados() {
  if (nome.value !== '') {
    if (login.value !== '') {
      if(senha.value !== '') {
        emit('atualizar', {
          cod_arbitro: props.arbitro.cod_arbitro,
          nome_arbitro: nome.value,
          login_arbitro: login.value,
          senha_arbitro: senha.value
        })
